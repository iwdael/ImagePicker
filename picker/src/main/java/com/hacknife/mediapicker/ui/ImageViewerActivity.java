...
public class ImageViewerActivity extends ImageBaseActivity {
    ...
    private void initData() {
        mImages = MediaPicker.getInstance().getViewerItem();
        mPosition = getIntent().getIntExtra(MediaPicker.EXTRA_SELECTED_IMAGE_POSITION, 0);
        if (mImages.size() > 1)
            isMultiPhoto = true;
    ...
    private void initViewPager() {
        ...
        MediaPicker.getInstance().viewerItem(null);
    ...
    public void finishAfterTransition() {
        ...
        intent.putExtra(MediaPicker.EXTRA_EXIT_POSITION, current);
